...
if ($_GET['albumName'] != '') {
	...
	$db->query("INSERT INTO photoAlbums VALUES (NULL,".$row_currentUser['websiteID'].", '".$_GET['albumName']."')");
	$lastInsertedId = mysqli_insert_id($db);
	header("Location: albums-photos.php?albumID=".$lastInsertedId);	
}
else {
	//no need to add a new album
	header("Location: listings.php?action=added");	
}
?>
<?php
